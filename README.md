...
The server starts on port `3005` by default. Visit `http://localhost:3005/api/admin` to access the Admin Dashboard.
...
Install Nginx and configure a proxy to port 3005:
...
```
http://your-server:3005/api/admin
```
...
- Enter your backend server URL (e.g., `http://your-vps-ip:3005` or `https://your-domain.com`)
- Click **Save Settings**
...
```js
const CONFIG = {
  SERVER_URL: "http://localhost:3005",  // Override via popup
...
| POST | `/api/sandbox/process` | Sandbox processing for testing |
...
**JSON body:**
- `templateBytes` — number[] of the template file bytes
- `dataRows` — array of { username, password, email, email_password }
- `columnMap` — object mapping field names to column letters (e.g., { "username": "A", "password": "B" })
